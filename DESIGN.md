---
version: alpha
name: BrandGuard-design-system
description: "An Airtable-inspired B2B SaaS review interface anchored on white canvas, dark-ink type, hairline dividers, and sparse signature surfaces. BrandGuard should feel like a sober workflow product for marketing, PR, brand, and legal teams: calm by default, dense where review work requires it, and visually strongest on the image risk review surface. Primary actions use a near-black rounded CTA; secondary actions sit in a white outlined button. Type uses modest weights and zero letter-spacing."
...
colors:
  ...
  pricing-ink: "#1d1f25"
  risk-low-bg: "#eef8f0"
  risk-low-text: "#006400"
  risk-medium-bg: "#fff4cf"
  risk-medium-text: "#6f4a00"
  risk-high-bg: "#fff0e8"
  risk-high-text: "#aa2d00"
  risk-critical-bg: "#181d26"
  risk-critical-text: "#ffffff"
  review-canvas: "#181d26"
  review-canvas-panel: "#1d1f25"
  review-overlay-hand: "#aa2d00"
  review-overlay-ocr: "#254fad"
  review-overlay-symbol: "#0a2e0e"
...
components:
  ...
  filter-control:
    ...
    padding: 10px 12px
  review-canvas-panel:
    backgroundColor: "{colors.review-canvas-panel}"
    textColor: "{colors.on-dark}"
    typography: "{typography.body-md}"
    rounded: "{rounded.lg}"
    padding: 24px
  ...
  risk-badge-low:
    ...
    textColor: "{colors.risk-low-text}"
    typography: "{typography.caption}"
    rounded: "{rounded.md}"
    padding: 4px 8px
  risk-badge-medium:
    backgroundColor: "{colors.risk-medium-bg}"
    textColor: "{colors.risk-medium-text}"
    typography: "{typography.caption}"
    rounded: "{rounded.md}"
    padding: 4px 8px
  risk-badge-high:
    backgroundColor: "{colors.risk-high-bg}"
    textColor: "{colors.risk-high-text}"
    typography: "{typography.caption}"
    rounded: "{rounded.md}"
    padding: 4px 8px
  risk-badge-critical:
    backgroundColor: "{colors.risk-critical-bg}"
    textColor: "{colors.risk-critical-text}"
    typography: "{typography.caption}"
    rounded: "{rounded.md}"
    padding: 4px 8px
  overlay-region-hand:
    backgroundColor: "{colors.review-overlay-hand}"
    textColor: "{colors.on-primary}"
    typography: "{typography.caption}"
    rounded: "{rounded.md}"
    padding: 4px 8px
  overlay-region-ocr:
    backgroundColor: "{colors.review-overlay-ocr}"
    textColor: "{colors.on-primary}"
    typography: "{typography.caption}"
    rounded: "{rounded.md}"
    padding: 4px 8px
  overlay-region-symbol:
    backgroundColor: "{colors.review-overlay-symbol}"
    textColor: "{colors.on-primary}"
    typography: "{typography.caption}"
    rounded: "{rounded.md}"
    padding: 4px 8px
  demo-grid-card-peach:
    backgroundColor: "{colors.signature-peach}"
    textColor: "{colors.ink}"
    typography: "{typography.label-md}"
    rounded: "{rounded.md}"
    padding: 16px
  demo-grid-card-mint:
    backgroundColor: "{colors.signature-mint}"
    textColor: "{colors.ink}"
    typography: "{typography.label-md}"
    rounded: "{rounded.md}"
    padding: 16px
  demo-grid-card-yellow:
    backgroundColor: "{colors.signature-yellow}"
    textColor: "{colors.ink}"
    typography: "{typography.label-md}"
    rounded: "{rounded.md}"
    padding: 16px
  demo-grid-card-mustard:
    backgroundColor: "{colors.signature-mustard}"
    textColor: "{colors.ink}"
    typography: "{typography.label-md}"
    rounded: "{rounded.md}"
    padding: 16px
  article-rainbow-hero:
...
## BrandGuard Product Adaptation

This file uses Airtable's design language as a source reference, but BrandGuard is an operational B2B SaaS product. For app screens, this section overrides source-specific marketing guidance below.

### Product Personality
- BrandGuard should feel calm, trustworthy, and review-oriented.
- The UI should help marketing, PR, brand, and legal teams scan evidence quickly.
- Avoid promotional hero-page composition inside the product shell.
- Use a higher information density than the Airtable marketing pages, especially on dashboard, table, review, and audit-log screens.
- Keep the strongest visual moment on `/campaigns/[id]/review`, where the image canvas, overlays, risk evidence, and workflow controls work together.

### App Surfaces
- Use `{colors.canvas}` as the default page background.
- Use `{colors.surface-soft}` for filter areas, secondary panels, empty states, and quiet grouped controls.
- Use `{colors.review-canvas}` and `{colors.review-canvas-panel}` only for image review, overlay inspection, and analysis-focused surfaces.
- Use `{colors.signature-coral}`, `{colors.signature-forest}`, and `{colors.signature-cream}` sparingly for large callouts or empty states, not for routine dashboards.
- Do not use gradient, mesh, glow, or decorative background effects.

### Product Components
- Use `{component.app-shell}` for the authenticated app frame.
- Use `{component.app-sidebar}` for route navigation and team context.
- Use `{component.page-header}` for page titles, primary actions, and compact status context.
- Use `{component.metric-card}` for dashboard summaries such as pending reviews, high-risk campaigns, approved campaigns, and average risk score.
- Use `{component.data-table}` for campaign lists, risk dictionary entries, cases, and audit-style data.
- Use `{component.filter-control}` for search, select filters, date filters, and sort controls.
- Use `{component.review-canvas-panel}` for the uploaded image, overlays, OCR regions, and hand landmark visualization.
- Use `{component.review-side-panel}` for risk score, category findings, suggested revisions, comments, and workflow actions.
- Use `{component.workflow-panel}` for approval, revision request, rejection, and status transition controls.
- Use `{component.audit-log-row}` for state-change history.

### Risk Communication
- Risk states must combine color, text, and icon/shape. Never communicate risk level by color alone.
- Low risk uses `{colors.risk-low-bg}` and `{colors.risk-low-text}`.
- Medium risk uses `{colors.risk-medium-bg}` and `{colors.risk-medium-text}`.
- High risk uses `{colors.risk-high-bg}` and `{colors.risk-high-text}`.
- Critical risk uses `{colors.risk-critical-bg}` and `{colors.risk-critical-text}`.
- Risk wording must stay neutral: "검토 필요", "오탐 가능성", "맥락 확인 필요", "사람 검토 권장".
- Do not use deterministic or identity-labeling language in visual design, badges, empty states, charts, or tooltips.

### Review Overlay Colors
- Hand or visual pattern candidate overlays use `{colors.review-overlay-hand}`.
- OCR text overlays use `{colors.review-overlay-ocr}`.
- Symbol/object overlays use `{colors.review-overlay-symbol}`.
- Overlay labels should include a visible text label and accessible list counterpart.
- Selected overlays may increase border width or background opacity, but should not introduce new accent colors.

### Layout Guidance
- Product pages should use constrained full-width bands and dense, scannable panels rather than oversized marketing tiles.
- Cards are allowed for repeated items, metric summaries, review panels, and modal surfaces.
- Avoid nesting cards inside cards.
- Tables should keep visible headers, row separators, keyboard focus, and row-level click affordances.
- The review page should use a two-pane layout on desktop: image review canvas left, risk/workflow panel right.
- On mobile, review page panels stack in this order: status summary, image canvas, selected finding, workflow actions, comments, audit log.

### Typography Rules
- Use the documented type sizes, but set CSS `letter-spacing: 0` globally unless a component explicitly requires a monospace/code style.
- Prefer size, color contrast, and spacing over heavier font weights.
- Dashboard, table, and review-panel text should remain compact and readable; do not use hero-scale type inside tool surfaces.

### Button Rules
- Use `{component.button-primary}` for one dominant action in a local workflow, such as "분석 요청", "검토 제출", or "승인".
- Use `{component.button-secondary}` for secondary actions such as "재분석", "초안 저장", or "목록으로".
- Destructive or rejecting actions may use neutral outline styling plus explicit text; avoid sensational red styling unless the action is truly destructive.
- Do not use `{rounded.pill}` for ordinary app buttons. It remains reserved for pricing or special commercial surfaces.

## Overview

Airtable's marketing surfaces are quietly editorial. The base atmosphere is white canvas, dark ink type, generous whitespace, and a near-black rounded CTA — nothing is fighting for attention until a section needs to. The brand voltage doesn't come from gradient washes or accent walls; it comes from **full-bleed signature cards** in `{colors.signature-coral}`, `{colors.signature-forest}`, and `{colors.surface-dark}` that punctuate long-scroll explainer pages every two or three screens. Between those signature bands, the page reads like a print magazine: a headline, supporting copy, a small image cluster, then breathing room.
...
### Brand & Accent
- **Primary** (`{colors.primary}` — #181d26): The dominant brand color. Used for the primary CTA background, h1/h2 display type, and the `{colors.surface-dark}` band. Not "blue, then black" — black IS the primary throughout the marketing system.
